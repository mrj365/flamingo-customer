...
export class PickupComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    if (userAddressEntity && this.authenticationService.isUserLoggedIn()) {
          ...
          this.ngZone.run(() => {
            ...
            this.locationService.getLocations(locationSearchDTO)
              .subscribe(locationListResult => {
                ...
                // this.router.navigateByUrl('pickup-redirect');

                if(this.locations && this.locations.length > 0) {
                  let locationId = this.locations[0].id;
                  this.router.navigate(['/pickup-redirect'], { queryParams: { locationId: locationId }});
                } else {
                  this.hideFirstSearch = true;
                }
  ...
  goToDateSelection() {
    this.shoppingCartService.addPickupService(this.locations[0].id).subscribe(() =>
      {
