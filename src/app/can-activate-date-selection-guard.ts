...
@Injectable()
export class CanActivateDateSelectionGuard implements CanActivate {
  ...
  constructor(private router: Router, private authService: AuthenticationService,
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        ...
        if(!this.authService.isUserLoggedIn()) { // If the use ris not logged in, take them to the locations screen
            ...
            return false;
        } else if (this.hasNoServices() && !environment.pickupProfile) { // If the user has no services they will need to go back to the locations screen
